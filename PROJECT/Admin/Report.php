			 ...
			 $selQry = "select * from tbl_product";
				 ...
				 $rs = $con->query($selQry);
                   while ($data = $rs->fetch_assoc()) 
				   {
					   ...
					   $selQry2 = "select COALESCE(SUM(cart_quantity),0) as qty from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id  inner join tbl_product p on p.product_id=c.product_id inner join tbl_subcategory s on s.subcategory_id=p.subcategory_id inner join tbl_category ca on ca.category_id=s.category_id where c.product_id='".$data["product_id"]."'";
					 // echo $selQry2;
					   if(isset($_POST["btn_search"]))
						{
							$selQry2 = $selQry2." and b.booking_date between '".$_POST["fdate"]."' and '".$_POST["tdate"]."'";
                            //echo $selQry2;
						}
						
						$rs2 = $con->query($selQry2);
