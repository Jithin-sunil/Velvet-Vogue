...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_booking where customer_id='" .$_SESSION["cid"]. "'and booking_status='0'";
                $rs = $con->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["booking_id"];
                
                $upQry1 = "update tbl_booking set booking_datetime=NOW(),booking_amount='".$amt."',booking_status='1' where customer_id='" .$_SESSION["cid"]. "'";
				$con->query($upQry1);
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where b.customer_id='" .$_SESSION["cid"]. "' and booking_status='0' and cart_status=0";
            $res = $con->query($sel);
